...
class ImageStorageDataSource {
  Future<List<String>> uploadImages({List<Uint8List> imageUint8ListArray, String uid, String collection}) async {
    ...
    try {
      ...
      throw FirebaseException(
          plugin: 'CPWare', message: 'ImageStorageDataSource - uploadImages - error: ${e.toString()}');
    }
    return pathList;
  }

  Future<ImageEntity> getImageUrl(String path) async {
    final fireStorage.Reference ref = FirebaseStorage.instance.ref().child(path);
    String uri = '';
    try {
      uri = await ref.getDownloadURL();
      return ImageEntity(path: path, uri: uri);
    } on Exception catch (e) {
      FirebaseException(plugin: 'CPWare', message: 'ImageStorageDataSource - getImageUrl - error: ${e.toString()}');
      return null;
    }
  }
  ...
  Future<void> deleteImage(String path) async {
    ...
    try {
      ...
      throw FirebaseException(
          plugin: 'CPWare', message: 'ImageStorageDataSource - deleteImage - error: ${e.toString()}');
    }
  }
}
