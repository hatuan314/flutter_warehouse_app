import 'package:flutterwarehouseapp/common/extensions/screen_utils_extensions.dart';

class ConfirmOtpConstants {
  static const String verifyPhoneTxt = 'Xác minh số điện thoại';
  static const String contentTxt = 'Hãy nhập mã số được gửi tới số điện thoại ';
  static const String resendOtpMsgTxt = 'Gửi lại mã OTP sau ';
  static const String resendOtpTxt = 'Gửi lại';

  static double otpFieldHeight = 68.h;
  static double otpFieldWidth = 45.w;
  static double otpFieldPaddingHorizontal = 45.w;
  static double bodyPaddingTop = 32.h;
  static double resendOtpPaddingTop = 32.h;
}